...
(function() {
    if (/.+:\/\/(avio|avso|avxo).+\/movie\/.+/.test(location.href)) {
    ...
    } else if (/.+:\/\/(avio|avso|avxo).+\/star\/.+/.test(location.href)) {
        var mteam_root = document.createElement('p');
        var mteam = document.createElement('a');
        mteam.innerHTML = '在 M-team 中搜索';
    ...
    } else if (/.+:\/\/(avio|avso|avxo).+\/genre\/.+/.test(location.href)) {
        ...
        if (/&search=(.+?)&/.test(location.href)) {
            ...
            mteam.href = '//avio.pw/cn/search/' + location.href.match(/&search=(.+?)&/)[1];
            mteam.target = '_blank';
        ...
        jQuery.each(list, function (index, line) {
            ...
            mteam.href = '//avio.pw/cn/search/' + line.querySelectorAll('a[href^="details.php"]')[0].title.match(/(.+?) /)[1];
            mteam.target = '_blank';
        ...
        if (/\sCensored&nbsp;/.test(document.documentElement.innerHTML)) {
            var mteam = document.createElement('a');
            mteam.innerHTML = '在 JAV 中搜索';
            mteam.href = '//avio.pw/cn/search/' + jQuery('#top').text().match(/(.+?) /)[1];
            mteam.target = '_blank';
